import { setWomenSpecial } from '@/redux/slice/product';
import { fetchWomensProducts } from '@/services/operations/product';
...
const WomenSpecial = () => {
  ...
  const {womenSpecial,allProducts} = useSelector((state)=>state.product);

  const fetchOnlyWomen = async() => {
   try {
    setLoading(true)
    const result = await fetchWomensProducts();
    dispatch(setWomenSpecial(result));
   } catch (error) {
