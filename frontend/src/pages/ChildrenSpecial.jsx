import { setChildrenSpecial } from "@/redux/slice/product";
import { fetchChildrensProducts } from "@/services/operations/product";
...
const ChildrenSpecial = () => {
  ...
  const { childrenSpecial, allProducts } = useSelector(
    (state) => state.product
  );

  const fetchOnlyChildren = async () => {
    try {
      setLoading(true);
      const result = await fetchChildrensProducts();
      dispatch(setChildrenSpecial(result));
    } catch (error) {
...
export default ChildrenSpecial;
